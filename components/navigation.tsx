...
import { useLanguage } from "@/contexts/language-context"
import { getTranslation } from "@/lib/translations"
import { getHomePath } from "@/lib/base-path"
...
export default function Navigation() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-sm border-b border-[color:var(--border)] transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <Link href="/" className="flex items-center space-x-2">
            <div className="relative w-36 h-12">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <a href="#services" onClick={(e) => handleAnchorClick(e, "#services")} className="nav-link">
              {getTranslation(language, "nav.services")}
            </a>
            <a href="#projects" onClick={(e) => handleAnchorClick(e, "#projects")} className="nav-link">
              {getTranslation(language, "nav.projects")}
            </a>
            {/* <a href="#blog" onClick={(e) => handleAnchorClick(e, "#blog")} className="nav-link">
              {getTranslation(language, "nav.blog")}
            </a> */}
            <a href="#contact" onClick={(e) => handleAnchorClick(e, "#contact")} className="nav-link">
              {getTranslation(language, "nav.contact")}
            </a>
        ...
        {isOpen && (
          <div className="md:hidden pb-4 space-y-2 bg-background/80 backdrop-blur-sm">
            <a href="#services" onClick={(e) => { handleAnchorClick(e, "#services"); setIsOpen(false) }} className="block nav-link">
              {getTranslation(language, "nav.services")}
            </a>
            <a href="#projects" onClick={(e) => { handleAnchorClick(e, "#projects"); setIsOpen(false) }} className="block nav-link">
              {getTranslation(language, "nav.projects")}
            </a>
            <a href="#blog" onClick={(e) => { handleAnchorClick(e, "#blog"); setIsOpen(false) }} className="block nav-link">
              {getTranslation(language, "nav.blog")}
            </a>
            <a href="#contact" onClick={(e) => { handleAnchorClick(e, "#contact"); setIsOpen(false) }} className="block nav-link">
              {getTranslation(language, "nav.contact")}
            </a>
          </div>
        )}
